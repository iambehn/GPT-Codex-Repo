...
from run import (
    ...
    run_inspect_conversation_archive_upload_manifest,
...
class RunTests(unittest.TestCase):
    ...
    def test_run_prepare_conversation_archive_upload_emits_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            body_path = root / "conversation.md"
            record_path = root / "record.json"
    ...
    def test_run_inspect_conversation_archive_upload_manifest_renders_compact_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            body_path = root / "conversation.md"
            record_path = root / "record.json"
            ...
            result = run_inspect_conversation_archive_upload_manifest(upload["output_path"])
            self.assertTrue(result["ok"])
    ...
    def test_cli_routes_to_run_decision_regression_goldsets(self) -> None:
        ...
        try:
            ...
            self.assertIn("Suite count: 9", stdout.getvalue())
        finally:
            sys.argv = original_argv
        ...
        try:
            ...
            with patch(
                ...
                return_value={
                    "ok": True,
                    "status": "ok",
                    "rendered_output": json.dumps({"suite_count": 9}, indent=2),
                },
            ) as mock_run:
                with redirect_stdout(stdout):
                    exit_code = run_main()
    ...
    def test_prepare_proxy_review_skips_malformed_proxy_sidecars(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            sidecar_root = root / "sidecars"
            media_root = root / "media"
            ...
            broken_sidecar = sidecar_root / "marvel_rivals" / "bravo.proxy_scan.json"
            broken_sidecar.parent.mkdir(parents=True, exist_ok=True)
            broken_sidecar.write_text(
                json.dumps(
                    {
                        "schema_version": "proxy_scan_v1",
                        "scan_id": "marvel_rivals-bravo",
                        "ok": True,
                        "game": "marvel_rivals",
                        "source": str(bravo_source.resolve()),
                        "signal_count": 1,
                        "window_count": 1,
                        "windows": [
                            {
                        ...
                        "sidecar_path": str(broken_sidecar.resolve()),
                    }
                ),
                encoding="utf-8",
            )
