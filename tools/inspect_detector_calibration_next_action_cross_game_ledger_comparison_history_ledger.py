...
def inspect_detector_calibration_next_action_cross_game_ledger_comparison_history_ledger(
    *,
    ledger: str | Path,
...
def _render_compact_text(ledger_path: Path, payload: dict[str, Any]) -> str:
    ...
    if isinstance(latest_row, dict):
        ...
        lines.append(f"Valid game count: {_display(latest_row.get('valid_game_count'))}")
        lines.append(f"Input error count: {_display(latest_row.get('input_error_count'))}")
        lines.append(f"Top game: {_display(latest_row.get('top_game'))}")
        lines.append(f"Top ok rate: {_display(latest_row.get('top_ok_rate'))}")
        lines.append(f"Top latest status: {_display(latest_row.get('top_latest_status'))}")
    else:
        lines.append("None")
    ...
    for row in rows:
        lines.append(
            f"- {_display(row.get('run_id'))} | {_display(row.get('status'))} | valid_game_count={_display(row.get('valid_game_count'))} | top_game={_display(row.get('top_game'))}"
        )
    return "\n".join(lines)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect detector calibration next-action cross-game ledger comparison history ledger"
    )
