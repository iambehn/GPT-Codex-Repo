...
def _build_report(
    ...
    *,
    ...
    result["release_gate_summary"] = _build_release_gate_summary(result, min_reviewed)
    if game is not None:
...
def _validate_calibration_report_contract(report: dict[str, Any]) -> None:
    ...
    if missing_fields:
        raise ValueError(f"invalid_proxy_calibration_report_contract: missing fields: {', '.join(missing_fields)}")
    if not isinstance(report.get("diagnostics"), dict):
        raise ValueError("invalid_proxy_calibration_report_contract: diagnostics must be a dict")
    if not isinstance(report.get("recommendations"), dict):
        raise ValueError("invalid_proxy_calibration_report_contract: recommendations must be a dict")
    if not isinstance(report.get("warnings"), list):
        raise ValueError("invalid_proxy_calibration_report_contract: warnings must be a list")
    if not isinstance(report.get("skipped_counts"), dict):
        raise ValueError("invalid_proxy_calibration_report_contract: skipped_counts must be a dict")
    gate = report.get("release_gate_summary")
    if not isinstance(gate, dict):
        raise ValueError("invalid_proxy_calibration_report_contract: release_gate_summary must be a dict")
    missing_gate_fields = [field for field in REQUIRED_CALIBRATION_GATE_FIELDS if field not in gate]
    if missing_gate_fields:
        raise ValueError(
            f"invalid_proxy_calibration_report_contract: release_gate_summary missing fields: {', '.join(missing_gate_fields)}"
        )
    if str(gate.get("status")) not in {"pass", "fail"}:
        raise ValueError("invalid_proxy_calibration_report_contract: release_gate_summary.status must be pass or fail")
